...
<div class="wrapper">
  ...
  <div class="header">
    ...
    <button class="btn-pause hidden" id="btnPause">⏸</button>
  ...
  <div class="canvas-wrap">
    ...
    <div class="overlay" id="startScreen">
      ...
      <div class="option-row">
        ...
        <div class="two-btn">
          <button id="wallsOn"  class="on" data-i18n="yes">SÍ</button>
          <button id="wallsOff"            data-i18n="no">NO</button>
        </div>
      </div>

      <button class="btn"        id="startBtn"   data-i18n="start">▶ COMENZAR</button>
      <button class="btn yellow sm" id="rankingBtn" data-i18n="ranking_btn">🏆 RANKING</button>
    </div>

    <!-- Pausa -->
    <div class="overlay hidden" id="pauseScreen">
      <div class="ov-title yellow" data-i18n="paused">⏸ PAUSA</div>
      <div class="final-score" id="pauseInfo"></div>
      <button class="btn"    id="resumeBtn"        data-i18n="resume">▶ CONTINUAR</button>
      <button class="btn sm" id="menuFromPauseBtn" data-i18n="main_menu">MENÚ PRINCIPAL</button>
    </div>

    <!-- Top 10 — guardar nombre -->
    <div class="overlay hidden" id="nameScreen">
      <div class="ov-title yellow" data-i18n="top10_title">🏆 TOP 10</div>
      <div class="name-wrap">
        <p data-i18n="name_prompt">¡Entraste al ranking!<br>Introduce tu nombre:</p>
        <input class="name-input" id="nameInput" maxlength="12" placeholder="NOMBRE" autocomplete="off" spellcheck="false" />
        <button class="btn" id="saveNameBtn" data-i18n="save">💾 GUARDAR</button>
      </div>
    </div>
    ...
    <div class="overlay hidden" id="gameOverScreen">
      ...
      <button class="btn"        id="restartBtn"           data-i18n="play_again">▶ JUGAR DE NUEVO</button>
      <button class="btn sm"     id="menuFromGameOverBtn"  data-i18n="main_menu">MENÚ PRINCIPAL</button>
      <button class="btn yellow sm" id="rankingFromGameOverBtn" data-i18n="ranking_btn">🏆 RANKING</button>
    </div>

    <!-- Ranking -->
    <div class="overlay hidden" id="rankingScreen">
      <div class="ov-title yellow" data-i18n="ranking_title">🏆 RANKING</div>
      <div class="level-tabs" id="levelTabs"></div>
      <div class="ranking-wrap" id="rankingContent"></div>
      <button class="btn sm" id="closeRankingBtn" data-i18n="close">✕ CERRAR</button>
    </div>
  </div>
...
<script>
  ...
  const LANGS = {
    es: {
      ...
      yes: 'SÍ',        no: 'NO',          start: '▶ COMENZAR',     ranking_btn: '🏆 RANKING',
      ranking_title: '🏆 RANKING',         paused: '⏸ PAUSA',       resume: '▶ CONTINUAR',
      main_menu: 'MENÚ PRINCIPAL',         game_over: 'GAME OVER',  play_again: '▶ JUGAR DE NUEVO',
      close: '✕ CERRAR',                   top10_title: '🏆 TOP 10',
      name_prompt: '¡Entraste al ranking!\nIntroduce tu nombre:',
      save: '💾 GUARDAR',  level: 'NIVEL',  player: 'JUGADOR',
      score_col: 'PTS',    date_col: 'FECHA',  no_scores: 'Sin puntuaciones aún.',
      score_label: 'PUNTOS',  time_label: 'TIEMPO',  name_placeholder: 'NOMBRE',
    },
    en: {
      score: 'SCORE',   time: 'TIME',      speed_lbl: 'SPEED',      walls_lbl: 'WALLS',
      yes: 'YES',       no: 'NO',          start: '▶ START',        ranking_btn: '🏆 RANKING',
      ranking_title: '🏆 RANKING',         paused: '⏸ PAUSED',      resume: '▶ RESUME',
      main_menu: 'MAIN MENU',              game_over: 'GAME OVER',  play_again: '▶ PLAY AGAIN',
      close: '✕ CLOSE',                    top10_title: '🏆 TOP 10',
      name_prompt: "You made the ranking!\nEnter your name:",
      save: '💾 SAVE',     level: 'LEVEL',  player: 'PLAYER',
  ...
  function tick() {
    // saca el siguiente giro de la cola si hay
  ...
  // buffer de 2: no se pierden giros rápidos
  const dirQueue = [];

  function changeDir(d) {
    if (!running || paused || gameOver) return;
    const ref = dirQueue.length > 0 ? dirQueue[dirQueue.length - 1] : dir;
    if (d.x === -ref.x && d.y === -ref.y) return; // 180° no
    if (d.x ===  ref.x && d.y ===  ref.y) return; // ya va en esa dirección
  ...
  // si el toque viene del dpad, el swipe no hace nada
  ...
  document.querySelectorAll('.dpad-btn').forEach(btn => {
    ...
    btn.addEventListener('touchend', () => {
      setTimeout(() => { dpadTouched = false; }, 50);
    }, { passive: true });
